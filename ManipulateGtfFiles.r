...
class GFFProcessor:
    ...
    def attribute_todf(self,row):
        ...
        self.attributes_list.append(Dict)
        return Dict
    
    def process_gff_file(self):
            annotation = gffpd.read_gff3(self.gff_file)
            ...
            annotation_df['Result'] = annotation_df.apply(self.attribute_todf, axis=1)
            annotation_df=pd.DataFrame(self.attributes_list)
            annotation_df=annotation_df.drop(['level','hgnc_id','havana_gene','tag'],axis=1)
            return annotation_df


gff_file = 'gencode.v36.annotation.gtf'
gff_processor = GFFProcessor(gff_file)
annotation_df = gff_processor.process_gff_file()
annotation_df2=annotation_df[["gene_id","gene_name","gene_type"]]
annotation_df2.columns=['ensembl_gene_id','gene_name','gene_biotype']
annotation_df2=annotation_df2.set_index("ensembl_gene_id")
